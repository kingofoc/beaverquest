    ...
    {footerItems.map(({ href, label, activeIcon, icon }) => {
     ...
     return (
      <Link 
       key={label} 
       href={href} 
       className={`flex flex-col justify-center items-center text-center p-1 ${isActive ? 'tertiary-bg-faded scale-105 rounded-full' : 'bg-transparent'} transition duration-200 ease-in-out`}>
