...
export default function Footer() {
 ...
 return (
  <div className="fixed bottom-0 left-0 w-full px-10 py-3">
   <div className="grid grid-cols-3 gap-8 border-2 border-amber-300 rounded-full p-1">
    {footerItems.map(({ href, label, icon }) => {
     ...
     return (
      <Link 
       key={label} 
       href={href} 
       className={`flex flex-col justify-center items-center text-center ${isActive ? 'bg-blue-400 scale-105 rounded-full' : 'bg-transparent'} transition-all duration-200 ease-in-out`}>
